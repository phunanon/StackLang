...
const DomLoad = () => {
    ...
    input.focus();
    const state = Object.assign(Object.assign(Object.assign(Object.assign({ input, span, div }, { stack: [], funcs: {}, loopStack: [] }), { func: [], ifDepth: 0 }), { loop: false, inFunc: false, inString: false, inRemark: false }), { print: (text) => {
            console.log(text);
...
const exe = (state) => (word) => {
    var _a;
    const { stack } = state;
    if (state.inRemark && word !== ")")
        return;
    if (word === "(") {
        state.inRemark = true;
        return;
    ...
    if (lquo || rquo || state.inString) {
        if (lquo && !state.inString)
            state.inString = true;
        if (rquo && state.inString)
    ...
    switch (word) {
        case "pick":
            stack.push((_a = stack.at(-stack.pop() - 1)) !== null && _a !== void 0 ? _a : Number.POSITIVE_INFINITY);
            break;
        case "pop":
